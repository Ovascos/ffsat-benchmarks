(set-info :smt-lib-version 2.6)
(set-info :category "crafted")
(set-logic QF_FF)
(define-sort FF0 () (_ FiniteField 52435875175126190479447740508185965837690552500527637822603658699938581184513))
(declare-fun _26_alt () FF0)
(declare-fun _25_alt () FF0)
(declare-fun _5_alt () FF0)
(declare-fun _24_alt () FF0)
(declare-fun _4_alt () FF0)
(declare-fun _23_alt () FF0)
(declare-fun _3_alt () FF0)
(declare-fun _22_alt () FF0)
(declare-fun _2_alt () FF0)
(declare-fun _21_alt () FF0)
(declare-fun _13_alt () FF0)
(declare-fun _20_alt () FF0)
(declare-fun _12_alt () FF0)
(declare-fun _19_alt () FF0)
(declare-fun _11_alt () FF0)
(declare-fun _18_alt () FF0)
(declare-fun _10_alt () FF0)
(declare-fun _17_alt () FF0)
(declare-fun _9_alt () FF0)
(declare-fun _16_alt () FF0)
(declare-fun _8_alt () FF0)
(declare-fun _15_alt () FF0)
(declare-fun _7_alt () FF0)
(declare-fun _14_alt () FF0)
(declare-fun _6_alt () FF0)
(declare-fun _1_alt () FF0)
(declare-fun _0_alt () FF0)
(declare-fun _26 () FF0)
(declare-fun _25 () FF0)
(declare-fun _5 () FF0)
(declare-fun _24 () FF0)
(declare-fun _4 () FF0)
(declare-fun _23 () FF0)
(declare-fun _3 () FF0)
(declare-fun _22 () FF0)
(declare-fun _2 () FF0)
(declare-fun _21 () FF0)
(declare-fun _13 () FF0)
(declare-fun _20 () FF0)
(declare-fun _12 () FF0)
(declare-fun _19 () FF0)
(declare-fun _11 () FF0)
(declare-fun _18 () FF0)
(declare-fun _10 () FF0)
(declare-fun _17 () FF0)
(declare-fun _9 () FF0)
(declare-fun _16 () FF0)
(declare-fun _8 () FF0)
(declare-fun _15 () FF0)
(declare-fun _7 () FF0)
(declare-fun _14 () FF0)
(declare-fun _6 () FF0)
(declare-fun _1 () FF0)
(declare-fun _0 () FF0)
(declare-fun out_alt () FF0)
(declare-fun out () FF0)
(assert 
  (let ((let0 (as ff52435875175126190479447740508185965837690552500527637822603658699938581184512 FF0)))
  (let ((let1 _26_alt))
  (let ((let2 (ff.mul let1 let0)))
  (let ((let3 _25_alt))
  (let ((let4 _5_alt))
  (let ((let5 (ff.add let4 let3 let2)))
  (let ((let6 (ff.mul let3 let4)))
  (let ((let7 (= let6 let5)))
  (let ((let8 (ff.mul let3 let0)))
  (let ((let9 _24_alt))
  (let ((let10 _4_alt))
  (let ((let11 (ff.add let10 let9 let8)))
  (let ((let12 (ff.mul let9 let10)))
  (let ((let13 (= let12 let11)))
  (let ((let14 (ff.mul let9 let0)))
  (let ((let15 _23_alt))
  (let ((let16 _3_alt))
  (let ((let17 (ff.add let16 let15 let14)))
  (let ((let18 (ff.mul let15 let16)))
  (let ((let19 (= let18 let17)))
  (let ((let20 (ff.mul let15 let0)))
  (let ((let21 _22_alt))
  (let ((let22 _2_alt))
  (let ((let23 (ff.add let22 let21 let20)))
  (let ((let24 (ff.mul let21 let22)))
  (let ((let25 (= let24 let23)))
  (let ((let26 (ff.mul let21 let0)))
  (let ((let27 _21_alt))
  (let ((let28 _13_alt))
  (let ((let29 (ff.add let28 let27 let26)))
  (let ((let30 (ff.mul let27 let28)))
  (let ((let31 (= let30 let29)))
  (let ((let32 (ff.mul let27 let0)))
  (let ((let33 _20_alt))
  (let ((let34 _12_alt))
  (let ((let35 (ff.add let34 let33 let32)))
  (let ((let36 (ff.mul let33 let34)))
  (let ((let37 (= let36 let35)))
  (let ((let38 (ff.mul let33 let0)))
  (let ((let39 _19_alt))
  (let ((let40 _11_alt))
  (let ((let41 (ff.add let40 let39 let38)))
  (let ((let42 (ff.mul let39 let40)))
  (let ((let43 (= let42 let41)))
  (let ((let44 (ff.mul let39 let0)))
  (let ((let45 _18_alt))
  (let ((let46 _10_alt))
  (let ((let47 (ff.add let46 let45 let44)))
  (let ((let48 (ff.mul let45 let46)))
  (let ((let49 (= let48 let47)))
  (let ((let50 (ff.mul let45 let0)))
  (let ((let51 _17_alt))
  (let ((let52 _9_alt))
  (let ((let53 (ff.add let52 let51 let50)))
  (let ((let54 (ff.mul let51 let52)))
  (let ((let55 (= let54 let53)))
  (let ((let56 (ff.mul let51 let0)))
  (let ((let57 _16_alt))
  (let ((let58 _8_alt))
  (let ((let59 (ff.add let58 let57 let56)))
  (let ((let60 (ff.mul let57 let58)))
  (let ((let61 (= let60 let59)))
  (let ((let62 (ff.mul let57 let0)))
  (let ((let63 _15_alt))
  (let ((let64 _7_alt))
  (let ((let65 (ff.add let64 let63 let62)))
  (let ((let66 (ff.mul let63 let64)))
  (let ((let67 (= let66 let65)))
  (let ((let68 (ff.mul let63 let0)))
  (let ((let69 _14_alt))
  (let ((let70 _6_alt))
  (let ((let71 (ff.add let70 let69 let68)))
  (let ((let72 (ff.mul let69 let70)))
  (let ((let73 (= let72 let71)))
  (let ((let74 (ff.mul let69 let0)))
  (let ((let75 _1_alt))
  (let ((let76 _0_alt))
  (let ((let77 (ff.add let76 let75 let74)))
  (let ((let78 (ff.mul let76 let75)))
  (let ((let79 (= let78 let77)))
  (let ((let80 (ff.mul let28 let28)))
  (let ((let81 (= let80 let28)))
  (let ((let82 (ff.mul let34 let34)))
  (let ((let83 (= let82 let34)))
  (let ((let84 (ff.mul let40 let40)))
  (let ((let85 (= let84 let40)))
  (let ((let86 (ff.mul let46 let46)))
  (let ((let87 (= let86 let46)))
  (let ((let88 (ff.mul let52 let52)))
  (let ((let89 (= let88 let52)))
  (let ((let90 (ff.mul let58 let58)))
  (let ((let91 (= let90 let58)))
  (let ((let92 (ff.mul let64 let64)))
  (let ((let93 (= let92 let64)))
  (let ((let94 (ff.mul let70 let70)))
  (let ((let95 (= let94 let70)))
  (let ((let96 (ff.mul let4 let4)))
  (let ((let97 (= let96 let4)))
  (let ((let98 (ff.mul let10 let10)))
  (let ((let99 (= let98 let10)))
  (let ((let100 (ff.mul let16 let16)))
  (let ((let101 (= let100 let16)))
  (let ((let102 (ff.mul let22 let22)))
  (let ((let103 (= let102 let22)))
  (let ((let104 (ff.mul let75 let75)))
  (let ((let105 (= let104 let75)))
  (let ((let106 (ff.mul let76 let76)))
  (let ((let107 (= let106 let76)))
  (let ((let108 (and let107 let105 let103 let101 let99 let97 let95 let93 let91 let89 let87 let85 let83 let81 let79 let73 let67 let61 let55 let49 let43 let37 let31 let25 let19 let13 let7)))
  (let ((let109 _26))
  (let ((let110 (ff.mul let109 let0)))
  (let ((let111 _25))
  (let ((let112 _5))
  (let ((let113 (ff.add let112 let111 let110)))
  (let ((let114 (ff.mul let111 let112)))
  (let ((let115 (= let114 let113)))
  (let ((let116 (ff.mul let111 let0)))
  (let ((let117 _24))
  (let ((let118 _4))
  (let ((let119 (ff.add let118 let117 let116)))
  (let ((let120 (ff.mul let117 let118)))
  (let ((let121 (= let120 let119)))
  (let ((let122 (ff.mul let117 let0)))
  (let ((let123 _23))
  (let ((let124 _3))
  (let ((let125 (ff.add let124 let123 let122)))
  (let ((let126 (ff.mul let123 let124)))
  (let ((let127 (= let126 let125)))
  (let ((let128 (ff.mul let123 let0)))
  (let ((let129 _22))
  (let ((let130 _2))
  (let ((let131 (ff.add let130 let129 let128)))
  (let ((let132 (ff.mul let129 let130)))
  (let ((let133 (= let132 let131)))
  (let ((let134 (ff.mul let129 let0)))
  (let ((let135 _21))
  (let ((let136 _13))
  (let ((let137 (ff.add let136 let135 let134)))
  (let ((let138 (ff.mul let135 let136)))
  (let ((let139 (= let138 let137)))
  (let ((let140 (ff.mul let135 let0)))
  (let ((let141 _20))
  (let ((let142 _12))
  (let ((let143 (ff.add let142 let141 let140)))
  (let ((let144 (ff.mul let141 let142)))
  (let ((let145 (= let144 let143)))
  (let ((let146 (ff.mul let141 let0)))
  (let ((let147 _19))
  (let ((let148 _11))
  (let ((let149 (ff.add let148 let147 let146)))
  (let ((let150 (ff.mul let147 let148)))
  (let ((let151 (= let150 let149)))
  (let ((let152 (ff.mul let147 let0)))
  (let ((let153 _18))
  (let ((let154 _10))
  (let ((let155 (ff.add let154 let153 let152)))
  (let ((let156 (ff.mul let153 let154)))
  (let ((let157 (= let156 let155)))
  (let ((let158 (ff.mul let153 let0)))
  (let ((let159 _17))
  (let ((let160 _9))
  (let ((let161 (ff.add let160 let159 let158)))
  (let ((let162 (ff.mul let159 let160)))
  (let ((let163 (= let162 let161)))
  (let ((let164 (ff.mul let159 let0)))
  (let ((let165 _16))
  (let ((let166 _8))
  (let ((let167 (ff.add let166 let165 let164)))
  (let ((let168 (ff.mul let165 let166)))
  (let ((let169 (= let168 let167)))
  (let ((let170 (ff.mul let165 let0)))
  (let ((let171 _15))
  (let ((let172 _7))
  (let ((let173 (ff.add let172 let171 let170)))
  (let ((let174 (ff.mul let171 let172)))
  (let ((let175 (= let174 let173)))
  (let ((let176 (ff.mul let171 let0)))
  (let ((let177 _14))
  (let ((let178 _6))
  (let ((let179 (ff.add let178 let177 let176)))
  (let ((let180 (ff.mul let177 let178)))
  (let ((let181 (= let180 let179)))
  (let ((let182 (ff.mul let177 let0)))
  (let ((let183 _1))
  (let ((let184 _0))
  (let ((let185 (ff.add let184 let183 let182)))
  (let ((let186 (ff.mul let184 let183)))
  (let ((let187 (= let186 let185)))
  (let ((let188 (ff.mul let136 let136)))
  (let ((let189 (= let188 let136)))
  (let ((let190 (ff.mul let142 let142)))
  (let ((let191 (= let190 let142)))
  (let ((let192 (ff.mul let148 let148)))
  (let ((let193 (= let192 let148)))
  (let ((let194 (ff.mul let154 let154)))
  (let ((let195 (= let194 let154)))
  (let ((let196 (ff.mul let160 let160)))
  (let ((let197 (= let196 let160)))
  (let ((let198 (ff.mul let166 let166)))
  (let ((let199 (= let198 let166)))
  (let ((let200 (ff.mul let172 let172)))
  (let ((let201 (= let200 let172)))
  (let ((let202 (ff.mul let178 let178)))
  (let ((let203 (= let202 let178)))
  (let ((let204 (ff.mul let112 let112)))
  (let ((let205 (= let204 let112)))
  (let ((let206 (ff.mul let118 let118)))
  (let ((let207 (= let206 let118)))
  (let ((let208 (ff.mul let124 let124)))
  (let ((let209 (= let208 let124)))
  (let ((let210 (ff.mul let130 let130)))
  (let ((let211 (= let210 let130)))
  (let ((let212 (ff.mul let183 let183)))
  (let ((let213 (= let212 let183)))
  (let ((let214 (ff.mul let184 let184)))
  (let ((let215 (= let214 let184)))
  (let ((let216 (and let215 let213 let211 let209 let207 let205 let203 let201 let199 let197 let195 let193 let191 let189 let187 let181 let175 let169 let163 let157 let151 let145 let139 let133 let127 let121 let115)))
  (let ((let217 out_alt))
  (let ((let218 out))
  (let ((let219 (= let218 let217)))
  (let ((let220 (not let219)))
  (let ((let221 (= let142 let34)))
  (let ((let222 (= let154 let46)))
  (let ((let223 (= let148 let40)))
  (let ((let224 (= let160 let52)))
  (let ((let225 (= let166 let58)))
  (let ((let226 (= let124 let16)))
  (let ((let227 (= let130 let22)))
  (let ((let228 (= let136 let28)))
  (let ((let229 (= let118 let10)))
  (let ((let230 (= let184 let76)))
  (let ((let231 (= let112 let4)))
  (let ((let232 (= let183 let75)))
  (let ((let233 (= let172 let64)))
  (let ((let234 (= let178 let70)))
  (let ((let235 (and let234 let233 let232 let231 let230 let229 let228 let227 let226 let225 let224 let223 let222 let221)))
  (let ((let236 (and let235 let220 let216 let108)))
  let236
)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
)
(check-sat)
