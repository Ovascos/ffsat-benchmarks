(set-info :smt-lib-version 2.6)
(set-info :category "crafted")
(set-logic QF_FFA)
(define-sort FF0 () (_ FiniteField 52435875175126190479447740508185965837690552500527637822603658699938581184513))
(declare-fun out_alt () FF0)
(declare-fun _22_alt () FF0)
(declare-fun _3_alt () FF0)
(declare-fun _21_alt () FF0)
(declare-fun _6_alt () FF0)
(declare-fun _20_alt () FF0)
(declare-fun _13_alt () FF0)
(declare-fun _19_alt () FF0)
(declare-fun _9_alt () FF0)
(declare-fun _18_alt () FF0)
(declare-fun _8_alt () FF0)
(declare-fun _17_alt () FF0)
(declare-fun _7_alt () FF0)
(declare-fun _16_alt () FF0)
(declare-fun _5_alt () FF0)
(declare-fun _15_alt () FF0)
(declare-fun _4_alt () FF0)
(declare-fun _14_alt () FF0)
(declare-fun _2_alt () FF0)
(declare-fun _0_alt () FF0)
(declare-fun _1_alt () FF0)
(declare-fun _12_alt () FF0)
(declare-fun out () FF0)
(declare-fun _22 () FF0)
(declare-fun _3 () FF0)
(declare-fun _21 () FF0)
(declare-fun _6 () FF0)
(declare-fun _20 () FF0)
(declare-fun _13 () FF0)
(declare-fun _19 () FF0)
(declare-fun _9 () FF0)
(declare-fun _18 () FF0)
(declare-fun _8 () FF0)
(declare-fun _17 () FF0)
(declare-fun _7 () FF0)
(declare-fun _16 () FF0)
(declare-fun _5 () FF0)
(declare-fun _15 () FF0)
(declare-fun _4 () FF0)
(declare-fun _14 () FF0)
(declare-fun _2 () FF0)
(declare-fun _0 () FF0)
(declare-fun _1 () FF0)
(declare-fun _12 () FF0)
(assert 
  (let ((let0 out_alt))
  (let ((let1 _22_alt))
  (let ((let2 (= let1 let0)))
  (let ((let3 (as ff1 FF0)))
  (let ((let4 (as ff52435875175126190479447740508185965837690552500527637822603658699938581184512 FF0)))
  (let ((let5 _3_alt))
  (let ((let6 (ff.mul let5 let4)))
  (let ((let7 (ff.add let6 let3)))
  (let ((let8 _21_alt))
  (let ((let9 (ff.mul let8 let7)))
  (let ((let10 (= let9 let1)))
  (let ((let11 _6_alt))
  (let ((let12 (ff.mul let11 let4)))
  (let ((let13 (ff.add let12 let3)))
  (let ((let14 _20_alt))
  (let ((let15 (ff.mul let14 let13)))
  (let ((let16 (= let15 let8)))
  (let ((let17 _13_alt))
  (let ((let18 _19_alt))
  (let ((let19 (ff.mul let18 let17)))
  (let ((let20 (= let19 let14)))
  (let ((let21 _9_alt))
  (let ((let22 _18_alt))
  (let ((let23 (ff.mul let22 let21)))
  (let ((let24 (= let23 let18)))
  (let ((let25 _8_alt))
  (let ((let26 _17_alt))
  (let ((let27 (ff.mul let26 let25)))
  (let ((let28 (= let27 let22)))
  (let ((let29 _7_alt))
  (let ((let30 _16_alt))
  (let ((let31 (ff.mul let30 let29)))
  (let ((let32 (= let31 let26)))
  (let ((let33 _5_alt))
  (let ((let34 _15_alt))
  (let ((let35 (ff.mul let34 let33)))
  (let ((let36 (= let35 let30)))
  (let ((let37 _4_alt))
  (let ((let38 _14_alt))
  (let ((let39 (ff.mul let38 let37)))
  (let ((let40 (= let39 let34)))
  (let ((let41 _2_alt))
  (let ((let42 _0_alt))
  (let ((let43 (ff.mul let42 let41)))
  (let ((let44 (= let43 let38)))
  (let ((let45 _1_alt))
  (let ((let46 _12_alt))
  (let ((let47 (ff.mul let46 let45)))
  (let ((let48 (= let47 let17)))
  (let ((let49 (ff.mul let45 let5)))
  (let ((let50 (= let49 let46)))
  (let ((let51 (ff.mul let21 let21)))
  (let ((let52 (= let51 let21)))
  (let ((let53 (ff.mul let25 let25)))
  (let ((let54 (= let53 let25)))
  (let ((let55 (ff.mul let29 let29)))
  (let ((let56 (= let55 let29)))
  (let ((let57 (ff.mul let11 let11)))
  (let ((let58 (= let57 let11)))
  (let ((let59 (ff.mul let33 let33)))
  (let ((let60 (= let59 let33)))
  (let ((let61 (ff.mul let37 let37)))
  (let ((let62 (= let61 let37)))
  (let ((let63 (ff.mul let5 let5)))
  (let ((let64 (= let63 let5)))
  (let ((let65 (ff.mul let41 let41)))
  (let ((let66 (= let65 let41)))
  (let ((let67 (ff.mul let45 let45)))
  (let ((let68 (= let67 let45)))
  (let ((let69 (ff.mul let42 let42)))
  (let ((let70 (= let69 let42)))
  (let ((let71 (and let70 let68 let66 let64 let62 let60 let58 let56 let54 let52 let50 let48 let44 let40 let36 let32 let28 let24 let20 let16 let10 let2)))
  (let ((let72 out))
  (let ((let73 _22))
  (let ((let74 (= let73 let72)))
  (let ((let75 _3))
  (let ((let76 (ff.mul let75 let4)))
  (let ((let77 (ff.add let76 let3)))
  (let ((let78 _21))
  (let ((let79 (ff.mul let78 let77)))
  (let ((let80 (= let79 let73)))
  (let ((let81 _6))
  (let ((let82 (ff.mul let81 let4)))
  (let ((let83 (ff.add let82 let3)))
  (let ((let84 _20))
  (let ((let85 (ff.mul let84 let83)))
  (let ((let86 (= let85 let78)))
  (let ((let87 _13))
  (let ((let88 _19))
  (let ((let89 (ff.mul let88 let87)))
  (let ((let90 (= let89 let84)))
  (let ((let91 _9))
  (let ((let92 _18))
  (let ((let93 (ff.mul let92 let91)))
  (let ((let94 (= let93 let88)))
  (let ((let95 _8))
  (let ((let96 _17))
  (let ((let97 (ff.mul let96 let95)))
  (let ((let98 (= let97 let92)))
  (let ((let99 _7))
  (let ((let100 _16))
  (let ((let101 (ff.mul let100 let99)))
  (let ((let102 (= let101 let96)))
  (let ((let103 _5))
  (let ((let104 _15))
  (let ((let105 (ff.mul let104 let103)))
  (let ((let106 (= let105 let100)))
  (let ((let107 _4))
  (let ((let108 _14))
  (let ((let109 (ff.mul let108 let107)))
  (let ((let110 (= let109 let104)))
  (let ((let111 _2))
  (let ((let112 _0))
  (let ((let113 (ff.mul let112 let111)))
  (let ((let114 (= let113 let108)))
  (let ((let115 _1))
  (let ((let116 _12))
  (let ((let117 (ff.mul let116 let115)))
  (let ((let118 (= let117 let87)))
  (let ((let119 (ff.mul let115 let75)))
  (let ((let120 (= let119 let116)))
  (let ((let121 (ff.mul let91 let91)))
  (let ((let122 (= let121 let91)))
  (let ((let123 (ff.mul let95 let95)))
  (let ((let124 (= let123 let95)))
  (let ((let125 (ff.mul let99 let99)))
  (let ((let126 (= let125 let99)))
  (let ((let127 (ff.mul let81 let81)))
  (let ((let128 (= let127 let81)))
  (let ((let129 (ff.mul let103 let103)))
  (let ((let130 (= let129 let103)))
  (let ((let131 (ff.mul let107 let107)))
  (let ((let132 (= let131 let107)))
  (let ((let133 (ff.mul let75 let75)))
  (let ((let134 (= let133 let75)))
  (let ((let135 (ff.mul let111 let111)))
  (let ((let136 (= let135 let111)))
  (let ((let137 (ff.mul let115 let115)))
  (let ((let138 (= let137 let115)))
  (let ((let139 (ff.mul let112 let112)))
  (let ((let140 (= let139 let112)))
  (let ((let141 (and let140 let138 let136 let134 let132 let130 let128 let126 let124 let122 let120 let118 let114 let110 let106 let102 let98 let94 let90 let86 let80 let74)))
  (let ((let142 (= let72 let0)))
  (let ((let143 (not let142)))
  (let ((let144 (= let99 let29)))
  (let ((let145 (= let81 let11)))
  (let ((let146 (= let75 let5)))
  (let ((let147 (= let112 let42)))
  (let ((let148 (= let111 let41)))
  (let ((let149 (= let107 let37)))
  (let ((let150 (= let103 let33)))
  (let ((let151 (= let115 let45)))
  (let ((let152 (= let91 let21)))
  (let ((let153 (= let95 let25)))
  (let ((let154 (and let153 let152 let151 let150 let149 let148 let147 let146 let145 let144)))
  (let ((let155 (and let154 let143 let141 let71)))
  let155
))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
)
(check-sat)
